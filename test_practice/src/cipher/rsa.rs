// encryption & decryption implmentation
use std::error::Error;
...
impl Rsa {
    pub fn new(input: String) -> Result<Self, Box<dyn Error>> {
        ...
        let public_key = private_key.to_public_key(); // get the PUBLIC key from the Private key
        let encrypted_data = public_key.encrypt(
            &mut rng, PaddingScheme::new_pkcs1v15_encrypt(), input_bytes
        )?;
...
impl super::Cipher for Rsa {
    fn original_string(&self) -> Result<String, Box<dyn Error>> {
        ...
        let decrypted_data = self.private_key.decrypt(
            PaddingScheme::new_pkcs1v15_encrypt(), &decoded_data
        )?;
